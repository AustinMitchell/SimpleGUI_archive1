...
public class Slider extends Widget {
	...
	public void Update() {
		...
		if (containsMouse() && Mouse.isPressed()) {
			if (isHorizontal) {
				value = scaleValue(Math.max(Math.min(Mouse.getX(), x+w-10), x+10), x+10, x+w-10, low, high);
			} else {
				value = scaleValue(Math.max(Math.min(Mouse.getY(), y+h-10), y+10), y+10, y+h-10, low, high);
			}
	...
	public void Draw() {
		...
		draw.setBorderColor(borderColor);
		if (!isHorizontal) {
